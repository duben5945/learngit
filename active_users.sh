...
printf "%-4s %-10s %-15s %-6s %-8s\n" "Rank" "User" "Start" "Logins" "Usage time"
...
do
	...
	firstlog=$(tail -n 1 /tmp/user.$$|awk '{print $((NF-5)),$((NF-4)),$((NF-3)) }')
	nlogin=$(cat /tmp/user.$$|wc -l )
	hours=$(echo "$minutes / 60.0"|bc)

	printf "%-10s %-15s %-6s %-8s\n" $user "$firstlog" $nlogin $hours
done < /tmp/users.$$
 
)|sort -nrk 6|awk '{printf("%-4s %s\n",NR,$0)}'
